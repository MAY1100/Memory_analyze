...
# Set the home directory for the case files to the current working directory
HOME=$(pwd)
...
# Function to handle results and generate a report
function RESULTS()
{
	...
	printf "${STOP}"
	sleep 1
	# Check for directories inside the forensic case directory
	...
	then
	 ...
	 # Zip the results report and forensic tools
	printf "${BOLD}${CYAN}"
...
# Function to run Volatility for memory analysis
function VOLATILITY_ANALYZE()
{
	...
	# Output the memory profile used for the analysis
